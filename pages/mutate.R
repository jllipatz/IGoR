...
.IGoR$page$mutate$sv <- function(input, output, session) {
  ...
  # arg0Type : input type
  # args     : additionnal args count
  # argsType : additionnal args type (- for any or not applicable, c)haracter, n)numeric), D)ate)
  # arg1     : title of 2nd arg field
  # arg2     : title of 3nd arg field
  # groups   : group_by meaningful?
  # narm     : -: not meaninful else default value
...
#    .IGoR$Zrename(c(none.fun='',f(c("any",class))))
# NB: First entry is empty, not 'none.fun'
    .IGoR$Zrename(list(none=c(none.fun=''),head.any=f("any"),head.class=f(class)))
  }
  ...
  output$mutate.fun <- renderUI(
    if (.isEQ(input$mutate.type,2)&&.isNotEmpty(input$mutate.old))
      selectizeInput("mutate.fun",label=.IGoR$s1(.IGoR$Z$any$fun),
        choices=functionsMenu(class(get(input$main.data,envir=.GlobalEnv)[[input$mutate.old]]))
      )
  )
              ...
              paste0(
                ...
                if (fun.args()>0)
                  paste0(
                    ...
                    if ((fun.argsType()=="c")&&(length(input$mutate.chr.arg1)>0))
                      paste0('"',input$mutate.chr.arg1,'"')
                    else
                    if ((fun.argsType()=="n")&&(length(input$mutate.num.arg1)>0))
                      input$mutate.num.arg1,
                    if (fun.args()>1)
                      if ((fun.argsType()=="c")&&(length(input$mutate.chr.arg2)>0))
                        paste0(',"',input$mutate.chr.arg2,'"')
                      else
                      if ((fun.argsType()=="n")&&(length(input$mutate.num.arg2)>0))
